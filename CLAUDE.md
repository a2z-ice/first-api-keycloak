...
# Login to ArgoCD (NodePort — no port-forward needed)
...
- **Kind cluster** configured in `cluster/kind-config.yaml` with NodePort 31111 (Keycloak), 32000 (legacy), 30000 (frontend), 30080/30081 (ArgoCD HTTP/HTTPS)
...
| ArgoCD UI (HTTP) | http://localhost:30080 | NodePort — redirects to HTTPS; no port-forward needed |
...
- **ArgoCD NodePort access**: `cluster/kind-config.yaml` now maps ports 30080 (HTTP) and 30081 (HTTPS) for ArgoCD — no `kubectl port-forward` needed. For clusters created **before this fix**, start Docker proxy containers instead of recreating the cluster:
  ```bash
  ...
  ```
  `cicd-pipeline-test.sh` uses `setup_argocd_login()` which connects to the NodePort directly — it will print the above commands if port 30080 is unreachable.
